    ...
    setMessages([
      {
        role: "assistant",
        text: openingMsg,
        chips: [t('bot.chips.idea'), t('bot.chips.website'), t('bot.chips.app')],
      },
    ]);
  }, [t]);
        ...
        if (fullText.includes("READY_TO_SEND:")) {
          ...
          if (match) {
            try {
              ...
              setMessages((prev) => [
                ...prev,
                { role: "assistant", text: "Let me confirm your details — can you review what we have?", chips: [] },
              ]);
            }
          }
        } else {
          ...
          setHistory((prev) => [...prev, { role: "assistant", content: fullText }]);
          setMessages((prev) => [...prev, { role: "assistant", text: mainText, chips }]);
        }
      } catch (err) {
      ...
      {open && (
        <div className="bot-panel">
          <div className="bot-header">
            ...
            <button className="bot-close" onClick={() => setOpen(false)} aria-label="Close">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
                <line x1="18" y1="6" x2="6" y2="18" />
                <line x1="6" y1="6" x2="18" y2="18" />
              </svg>
          ...
          <div className="bot-messages">
            {messages.map((msg, i) => (
              <div key={i} className={`bot-msg ${msg.role}`}>
                {msg.role === "assistant" && <div className="bot-msg-avatar">TW</div>}
                {msg.role === "user" && <div className="bot-msg-avatar user-av">You</div>}
                <div className={`bot-bubble${msg.success ? " bot-bubble-success" : ""}`}>
                  {msg.success && <div className="bot-success-icon">✓</div>}
                  <p>{msg.text}</p>
                  {msg.chips && msg.chips.length > 0 && (
                    <div className="bot-chips">
                      {msg.chips.map((chip, ci) => (
                        <button key={ci} className="bot-chip" onClick={() => sendMessage(chip)}>
                          {chip}
                        </button>
                      ))}
                    </div>
                ...
                <div className="bot-bubble">
                  ...
                  <div className="bot-confirm-card">
                    ...
                    {[
                      ...
                      value && value !== "undefined" && value !== "Not specified" ? (
                    ...
                    <div className="bot-confirm-actions">
                      <button className="bot-btn-confirm" onClick={handleConfirm}>
                        {t('bot.confirm.send')}
                      </button>
