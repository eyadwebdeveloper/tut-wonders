...
const Apply = () => {
  ...
  const EXPERT_OR_TRAINER_OPTIONS = t('apply.form.fields.expertOrTrainer.options', { returnObjects: true });
  ...
  const validate = () => {
    ...
    if (!form.expertOrTrainer) e.expertOrTrainer = t('apply.errors.expertOrTrainer');
      ...
      <div className="container">
        ...
        <div className="apply-header">
          ...
          <h2 className="section-title reveal" style={{ textAlign: 'center' }}>
            {t('apply.title')}<br />{t('apply.titleEnd')}
          </h2>
          ...
          <div className="apply-form-wrapper">
            ...
            {submitted ? (
              ...
              <div className="apply-success">
                ...
                <div className="apply-success-ref">{t('apply.success.ref').replace('{ref}', refCode)}</div>

                <div className="apply-success-email">
                  <p className="apply-success-email-title">📎 {t('apply.success.filesTitle')}</p>
                  <p dangerouslySetInnerHTML={{ 
                    __html: t('apply.success.filesMessage')
                  }} />
                  <p style={{ fontSize: '.75rem' }}>
                    Subject: <strong>{t('apply.success.subject').replace('{ref}', refCode)}</strong>
                  </p>
                  <a
              ...
              <>
                ...
                <div className="apply-form-title">
                  {t('apply.form.title')} <span>·</span> {new Date().getFullYear()}
                </div>

                <form onSubmit={handleSubmit} noValidate>

                  {/* ── Personal Details ── */}
                  <p className="apply-form-section">{t('apply.form.sections.personal')}</p>

                  <div className="apply-row">
                    <div className="apply-group">
                      <label className="apply-label">{t('apply.form.fields.firstName.label')} <span className="req">*</span></label>
                      <input
                        className={`apply-input${errors.firstName ? ' error' : ''}`}
                        name="firstName" value={form.firstName} onChange={handleChange}
                        placeholder={t('apply.form.fields.firstName.placeholder')}
                      />
                      {errors.firstName && <span className="apply-hint">{errors.firstName}</span>}
                    </div>
                    <div className="apply-group">
                      <label className="apply-label">{t('apply.form.fields.lastName.label')} <span className="req">*</span></label>
                      <input
                        className={`apply-input${errors.lastName ? ' error' : ''}`}
                        name="lastName" value={form.lastName} onChange={handleChange}
                        placeholder={t('apply.form.fields.lastName.placeholder')}
                      />
                  ...
                  <div className="apply-row">
                    <div className="apply-group">
                      <label className="apply-label">{t('apply.form.fields.email.label')} <span className="req">*</span></label>
                      <input
                    ...
                    <div className="apply-group">
                      <label className="apply-label">{t('apply.form.fields.phone.label')} <span className="req">*</span></label>
                      <input
                  ...
                  {/* ── Role & Experience ── */}
                  <p className="apply-form-section">{t('apply.form.sections.role')}</p>

                  <div className="apply-group">
                    <label className="apply-label">{t('apply.form.fields.role.label')} <span className="req">*</span></label>
                    <select
                      className={`apply-select${errors.role ? ' error' : ''}`}
                      name="role" value={form.role} onChange={handleChange}
                    >
                      <option value="">{t('apply.form.fields.role.placeholder')}</option>
                  ...
                  <div className="apply-group">
                    <label className="apply-label">{t('apply.form.fields.expertOrTrainer.label')} <span className="req">*</span></label>
                    <select
                      className={`apply-select${errors.expertOrTrainer ? ' error' : ''}`}
                      name="expertOrTrainer" value={form.expertOrTrainer} onChange={handleChange}
                    >
                      <option value="">{t('apply.form.fields.expertOrTrainer.placeholder')}</option>
                  ...
                  <div className="apply-row">
                    <div className="apply-group">
                      <label className="apply-label">{t('apply.form.fields.portfolio.label')}</label>
                      <input
                        className="apply-input"
                        type="url" name="portfolio" value={form.portfolio} onChange={handleChange}
                        placeholder={t('apply.form.fields.portfolio.placeholder')}
                      />
                    </div>
                    <div className="apply-group">
                      <label className="apply-label">{t('apply.form.fields.linkedin.label')}</label>
                      <input
                        className="apply-input"
                        type="url" name="linkedin" value={form.linkedin} onChange={handleChange}
                        placeholder={t('apply.form.fields.linkedin.placeholder')}
                      />
                    </div>
                  </div>

                  <div className="apply-group">
                    <label className="apply-label">{t('apply.form.fields.summary.label')} <span className="req">*</span></label>
                    <textarea
                    ...
                    <span style={{ fontSize: '.7rem', color: 'var(--text-dim)', textAlign: 'right', display: 'block', marginTop: '4px' }}>
                      {t('apply.charCount').replace('{count}', form.summary.length)}
                    </span>
                    {errors.summary && <span className="apply-hint">{errors.summary}</span>}
                  </div>

                  {/* ── Attachments notice ── */}
                  <p className="apply-form-section">{t('apply.form.sections.attachments')}</p>
                  ...
                  <div className="apply-email-notice">
                    ...
                    <p>
                      {t('apply.notice.title')} <strong style={{ color: 'var(--cream)' }}>{t('apply.notice.cv')}</strong> {t('apply.notice.and')}{' '}
                      <strong style={{ color: 'var(--cream)' }}>{t('apply.notice.id')}</strong> {t('apply.notice.to')}{' '}
