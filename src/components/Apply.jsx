...
/* ─── HELPERS ─────────────────────────────────────────────── */
const generateRef = () =>
  'TW-' + Date.now().toString(36).toUpperCase() + '-' + Math.random().toString(36).slice(2, 5).toUpperCase();
...
const EXPERT_OR_TRAINER_OPTIONS = [
  'Expert',
  'Trainer',
  'Both (Expert & Trainer)',
];
...
const PERKS = [
  {
    ...
    desc: 'Remote-first culture with async communication and flexible schedules.',
  },
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
    ...
    desc: 'Ship products used by thousands — not side exercises or fake demos.',
  },
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
    ...
    desc: 'Work alongside designers, developers, and strategists who care about craft.',
  },
];
...
const Apply = () => {
  const [form, setForm] = useState({
    ...
    role: '', expertOrTrainer: '', portfolio: '', linkedin: '',
    summary: '',
  });
  ...
  /* intersection reveals for perks */
  useEffect(() => {
    ...
    document.querySelectorAll('.apply-perk').forEach(el => observer.observe(el));
  ...
  const validate = () => {
    ...
    if (!form.expertOrTrainer) e.expertOrTrainer = 'Please select Expert or Trainer';
    if (!form.summary.trim() || form.summary.length < 60)
      e.summary = 'Please write at least 60 characters';
    return e;
  };
      ...
      <div className="container">
        ...
        <div className="apply-header">
          <div className="apply-badge">
            ...
            Careers at Tut Wonders
          </div>
          <h2 className="section-title reveal" style={{ textAlign: 'center' }}>
            Build the Future<br />With Us
          </h2>
          <p className="section-sub reveal" style={{ textAlign: 'center', maxWidth: '520px', margin: '0 auto' }}>
            We're looking for talented people who love what they do. If that's you, tell us about yourself.
          </p>
        </div>
        ...
        <div className="apply-grid">
          ...
          <div className="apply-sidebar">
            <div className="apply-perks">
              {PERKS.map((p, i) => (
                <div className="apply-perk" key={i} style={{ transitionDelay: `${i * 0.12}s` }}>
            ...
            <div className="apply-roles">
              <div className="apply-roles-head">Open Positions</div>
              {ROLES.map(r => (
          ...
          <div className="apply-form-wrapper">
            ...
            {submitted ? (
              ...
              <div className="apply-success">
                ...
                <p>
                  Thank you, {form.firstName}! Your application has been submitted.
                  Expect a reply within 3–5 business days.
                </p>
                <div className="apply-success-ref">Ref: {refCode}</div>

                <div className="apply-success-email">
                  <p className="apply-success-email-title">📎 One last step — send your files</p>
                  <p>
                    Please email your <strong>CV / Resume</strong> and{' '}
                    <strong>National ID (front &amp; back)</strong> to us.
                    Use your ref code as the subject so we can match it to your application.
                  </p>
                  <p style={{ fontSize: '.75rem' }}>
                    Subject: <strong>Application Files – {refCode}</strong>
                  </p>
                  <a
                    href={`mailto:info@tutwonders.com?subject=Application Files – ${refCode}&body=Hi, please find attached my CV and National ID for my application (Ref: ${refCode}).%0A%0AName: ${form.firstName} ${form.lastName}%0ARole: ${form.role}`}
                  >
              ...
              <>
                ...
                <div className="apply-progress">
                  {Array.from({ length: totalSteps }).map((_, i) => (
                    <div
                      ...
                      className={[
                        'apply-progress-step',
                        i < filled - 1 ? 'done' : i === filled - 1 ? 'active' : '',
                      ].filter(Boolean).join(' ')}
                    />
                  ))}
                </div>
                ...
                <div className="apply-form-title">
                  Application Form <span>·</span> {new Date().getFullYear()}
                </div>

                <form onSubmit={handleSubmit} noValidate>

                  {/* ── Personal Details ── */}
                  <p className="apply-form-section">Personal Details</p>

                  <div className="apply-row">
                    <div className="apply-group">
                      <label className="apply-label">First Name <span className="req">*</span></label>
                      <input
                        className={`apply-input${errors.firstName ? ' error' : ''}`}
                        name="firstName" value={form.firstName} onChange={handleChange}
                        placeholder="Ahmed"
                      />
                      {errors.firstName && <span className="apply-hint">{errors.firstName}</span>}
                    </div>
                    <div className="apply-group">
                      <label className="apply-label">Last Name <span className="req">*</span></label>
                      <input
                        className={`apply-input${errors.lastName ? ' error' : ''}`}
                        name="lastName" value={form.lastName} onChange={handleChange}
                        placeholder="Mohamed"
                      />
                      {errors.lastName && <span className="apply-hint">{errors.lastName}</span>}
                    </div>
                  </div>

                  <div className="apply-row">
                    <div className="apply-group">
                      <label className="apply-label">Email <span className="req">*</span></label>
                      <input
                        className={`apply-input${errors.email ? ' error' : ''}`}
                        type="email" name="email" value={form.email} onChange={handleChange}
                        placeholder="you@example.com"
                      />
                      {errors.email && <span className="apply-hint">{errors.email}</span>}
                    </div>
                    <div className="apply-group">
                      <label className="apply-label">Phone / WhatsApp <span className="req">*</span></label>
                      <input
                        className={`apply-input${errors.phone ? ' error' : ''}`}
                        type="tel" name="phone" value={form.phone} onChange={handleChange}
                        placeholder="+20 10 1234 5678"
                      />
                      {errors.phone && <span className="apply-hint">{errors.phone}</span>}
                    </div>
                  </div>

                  {/* ── Role & Experience ── */}
                  <p className="apply-form-section">Role & Experience</p>

                  <div className="apply-group">
                    <label className="apply-label">Position Applied For <span className="req">*</span></label>
                    <select
                    ...
                    {errors.role && <span className="apply-hint">{errors.role}</span>}
                  </div>

                  <div className="apply-group">
                    <label className="apply-label">Expert or Trainer? <span className="req">*</span></label>
                    <select
                      className={`apply-select${errors.expertOrTrainer ? ' error' : ''}`}
                      name="expertOrTrainer" value={form.expertOrTrainer} onChange={handleChange}
                    >
                    ...
                    {errors.expertOrTrainer && <span className="apply-hint">{errors.expertOrTrainer}</span>}
                  </div>

                  <div className="apply-row">
                    <div className="apply-group">
                      <label className="apply-label">Portfolio / GitHub</label>
                      <input
                  ...
                  <div className="apply-group">
                    <label className="apply-label">Professional Summary <span className="req">*</span></label>
                    <textarea
                      className={`apply-textarea${errors.summary ? ' error' : ''}`}
                      name="summary" value={form.summary} onChange={handleChange}
                      placeholder="Tell us about your experience, skills, and what you bring to the team…"
                    />
                    <span style={{ fontSize: '.7rem', color: 'var(--text-dim)', textAlign: 'right', display: 'block', marginTop: '4px' }}>
                      {form.summary.length} / 60+ chars
                    </span>
                    {errors.summary && <span className="apply-hint">{errors.summary}</span>}
                  </div>

                  {/* ── Attachments notice ── */}
                  <p className="apply-form-section">Attachments</p>
                  ...
                  <div className="apply-email-notice">
                    ...
                    <p>
                      After submitting, you'll be asked to email your <strong style={{ color: 'var(--cream)' }}>CV</strong> and{' '}
                      <strong style={{ color: 'var(--cream)' }}>National ID (front &amp; back)</strong> to{' '}
                      <a href="mailto:info@tutwonders.com">info@tutwonders.com</a>.
                      We'll give you a pre-filled email draft — one click to send. (please send from your email in cv and include any additional notes in the email body)
                    </p>
                  </div>

                  {errors._global && (
                    <p style={{ color: 'var(--terra-light)', fontSize: '.82rem', marginTop: '8px', textAlign: 'center' }}>
                      {errors._global}
                    </p>
                  )}
